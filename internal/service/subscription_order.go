...
func NewSubscriptionOrderService(db *gorm.DB, subscriptionPlanService *SubscriptionPlanService, userSubscriptionService *UserSubscriptionService, paymentService *PaymentService, couponService *CouponService) *SubscriptionOrderService {
...
type CreateSubscriptionOrderResponse struct {
	Order          *model.SubscriptionOrderResponse `json:"order"`
	PaymentRecord  *model.PaymentRecordResponse     `json:"payment_record"`
...
func (sos *SubscriptionOrderService) CreateSubscriptionOrder(ctx context.Context, req *CreateSubscriptionOrderRequest) (*CreateSubscriptionOrderResponse, error) {
	...
	if req.CouponCode != "" {
		...
		// Get coupon details for later use
		if appliedCouponTemp, err := sos.couponService.GetCouponByCode(ctx, req.CouponCode); err == nil {
			appliedCoupon = appliedCouponTemp
		}
	}
	...
	paymentReq := &CreatePaymentOrderRequest{
		UserID:              req.UserID,
	...
	if appliedCoupon != nil && couponDiscountAmount > 0 {
		if err := sos.couponService.UseCoupon(ctx, appliedCoupon.ID, uint64(req.UserID), uint64(order.ID), couponDiscountAmount, amount+setupFee, plan.Currency); err != nil {
			logger.Error("Failed to record coupon usage", 
				logger.Error2("error", err), 
				logger.Any("coupon_id", appliedCoupon.ID),
...
func (sos *SubscriptionOrderService) ProcessOrderPaymentSuccess(ctx context.Context, orderID uint) error {
	...
	case model.OrderTypeUpgrade, model.OrderTypeDowngrade:
		...
		if err := tx.Where("user_id = ? AND status IN (?)", 
			order.UserID, []string{model.UserSubscriptionStatusActive, model.UserSubscriptionStatusTrial}).
		...
		if order.OrderType == model.OrderTypeUpgrade {
		...
		if order.OrderType == model.OrderTypeUpgrade {
			...
			if err := tx.Model(&currentSubscription).Update("status", model.UserSubscriptionStatusCancelled).Error; err != nil {
				tx.Rollback()
...
func (sos *SubscriptionOrderService) GetSubscriptionOrder(ctx context.Context, orderID uint) (*model.SubscriptionOrder, error) {
	var order model.SubscriptionOrder
...
func (sos *SubscriptionOrderService) GetSubscriptionOrderByNumber(ctx context.Context, orderNumber string) (*model.SubscriptionOrder, error) {
	var order model.SubscriptionOrder
...
func (sos *SubscriptionOrderService) GetUserSubscriptionOrders(ctx context.Context, userID uint, limit, offset int) ([]*model.SubscriptionOrder, int64, error) {
	var orders []*model.SubscriptionOrder
	var totalCount int64

	// Get total count
	if err := sos.db.WithContext(ctx).Model(&model.SubscriptionOrder{}).
...
func calculateProration(currentSubscription *model.UserSubscription, currentPlan *model.SubscriptionPlan, newPlan *model.SubscriptionPlan) float64 {
	// Skip proration for lifetime subscriptions
	if currentPlan.BillingCycle == model.BillingCycleLifetime || newPlan.BillingCycle == model.BillingCycleLifetime {
		return 0.0
	}
...
func (sos *SubscriptionOrderService) CreateUpgradeDowngradeOrder(ctx context.Context, userID uint, req *UpgradeDowngradeRequest) (*CreateSubscriptionOrderResponse, error) {
	...
	if newPlan.Price > currentPlan.Price {
		...
		orderType = model.OrderTypeDowngrade
	} else {
...
func getApplyImmediately(orderType string, userPreference *bool) bool {
	...
	return orderType == model.OrderTypeUpgrade
}
...
func (sos *SubscriptionOrderService) GetOrdersWithFiltering(ctx context.Context, req *GetOrdersRequest) ([]*model.SubscriptionOrder, int64, error) {
	query := sos.db.WithContext(ctx).Model(&model.SubscriptionOrder{})
...
func (sos *SubscriptionOrderService) GetOrderWithRelations(ctx context.Context, orderID uint) (*model.SubscriptionOrder, error) {
	var order model.SubscriptionOrder
...
func (sos *SubscriptionOrderService) UpdateOrderStatus(ctx context.Context, orderID uint, status string, adminID uint, notes, reason string) (*model.SubscriptionOrder, error) {
...
func (sos *SubscriptionOrderService) UpdateOrderStatusWithEvidence(ctx context.Context, orderID uint, req *UpdateOrderStatusRequest, adminID uint) (*model.SubscriptionOrder, error) {
	// Start transaction
	tx := sos.db.WithContext(ctx).Begin()
	defer func() {
		if r := recover(); r != nil {
	...
	if req.Status == model.OrderStatusPaid && req.PaymentEvidence != nil {
	...
	if req.Status == model.OrderStatusPaid && order.PaidAt == nil {
	...
	if req.Status == model.OrderStatusPaid && order.Status != model.OrderStatusPaid {
...
func (sos *SubscriptionOrderService) validatePaymentEvidence(order *model.SubscriptionOrder, evidence *PaymentEvidence) error {
...
func (sos *SubscriptionOrderService) isCriticalOperation(oldStatus, newStatus string) bool {
	criticalChanges := map[string][]string{
		model.OrderStatusPending:   {model.OrderStatusPaid},                                    // Manual payment confirmation
		model.OrderStatusFailed:    {model.OrderStatusPaid},                                    // Retry failed payment
		model.OrderStatusCancelled: {model.OrderStatusPaid, model.OrderStatusPending},          // Reactivate cancelled order
		model.OrderStatusPaid:      {model.OrderStatusRefunded, model.OrderStatusCancelled},    // Reverse paid order
	}
...
func (sos *SubscriptionOrderService) ProcessRefund(ctx context.Context, req *ProcessRefundRequest) (*model.SubscriptionOrder, error) {
	// Start transaction
	tx := sos.db.WithContext(ctx).Begin()
	defer func() {
		if r := recover(); r != nil {
	...
	if order.RefundAmount+req.Amount >= order.TotalAmount {
		updateData["status"] = model.OrderStatusRefunded
	}
...
func (sos *SubscriptionOrderService) BulkUpdateOrders(ctx context.Context, req *BulkUpdateOrdersRequest) (*BulkUpdateResult, error) {
	...
	for _, orderID := range req.OrderIDs {
		...
		case "cancel":
			_, err = sos.UpdateOrderStatus(ctx, orderID, model.OrderStatusCancelled, req.ProcessedBy, req.Notes, req.Reason)
		case "refund":
...
func (sos *SubscriptionOrderService) checkDuplicateOrders(ctx context.Context, userID uint, planID uint, orderType string) error {
	...
	if err := sos.db.WithContext(ctx).Model(&model.SubscriptionOrder{}).
		Where("user_id = ? AND subscription_plan_id = ? AND order_type = ? AND status IN (?)", 
			userID, planID, orderType, []string{model.OrderStatusPending}).
	...
	if orderType == model.OrderTypeNew {
		var activeCount int64
		if err := sos.db.WithContext(ctx).Model(&model.UserSubscription{}).
...
func (sos *SubscriptionOrderService) checkRefundTimeWindow(order *model.SubscriptionOrder) error {
