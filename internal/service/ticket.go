...
func (s *TicketService) CreateTicket(ctx context.Context, userID uint, req *CreateTicketRequest) (*model.Ticket, error) {
	...
	ticket := &model.Ticket{
		...
		UserID:      userID,
		Tags:        stringToPtr(req.Tags),
		Metadata:    stringToPtr(req.Metadata),
	}
...
func (s *TicketService) GetTicket(ctx context.Context, ticketID uint) (*model.Ticket, error) {
...
func (s *TicketService) GetTicketByNumber(ctx context.Context, ticketNo string) (*model.Ticket, error) {
	var ticket model.Ticket
...
func (s *TicketService) GetTickets(ctx context.Context, req *GetTicketsRequest) ([]*model.Ticket, int64, error) {
...
func (s *TicketService) UpdateTicket(ctx context.Context, ticketID uint, req *UpdateTicketRequest) (*model.Ticket, error) {
	// Get existing ticket
...
func (s *TicketService) AssignTicket(ctx context.Context, ticketID uint, req *AssignTicketRequest) (*model.Ticket, error) {
	// Get existing ticket
	ticket, err := s.GetTicket(ctx, ticketID)
	if err != nil {
		return nil, err
	...
	if err := s.db.WithContext(ctx).First(&assignedUser, req.AssignedToID).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
	...
	if !assignedUser.IsAdmin() {
		return nil, fmt.Errorf("can only assign tickets to admin users")
	}
	...
	if ticket.Status == model.TicketStatusOpen {
		updates["status"] = model.TicketStatusInProgress
	}
...
func (s *TicketService) ResolveTicket(ctx context.Context, ticketID uint, resolvedByID uint, req *ResolveTicketRequest) (*model.Ticket, error) {
	// Get existing ticket
	ticket, err := s.GetTicket(ctx, ticketID)
	if err != nil {
		return nil, err
...
func (s *TicketService) CloseTicket(ctx context.Context, ticketID uint) (*model.Ticket, error) {
	// Get existing ticket
	ticket, err := s.GetTicket(ctx, ticketID)
	if err != nil {
		return nil, err
...
func (s *TicketService) GetTicketStats(ctx context.Context) (map[string]interface{}, error) {
	...
	if err := s.db.WithContext(ctx).Model(&model.Ticket{}).
	...
	// Get total count
	var totalCount int64
	if err := s.db.WithContext(ctx).Model(&model.Ticket{}).Count(&totalCount).Error; err != nil {
