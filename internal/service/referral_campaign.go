...
func NewReferralCampaignService(db *repository.Database) *ReferralCampaignService {
...
func (s *ReferralCampaignService) CreateReferralCampaign(ctx context.Context, adminUserID uint, req *CreateReferralCampaignRequest) (*model.ReferralCampaign, error) {
	// Check if code already exists
	var existingCampaign model.ReferralCampaign
...
func (s *ReferralCampaignService) GetReferralCampaigns(ctx context.Context, req *GetReferralCampaignsRequest) ([]*model.ReferralCampaign, int64, error) {
	var campaigns []*model.ReferralCampaign
	var total int64

	query := s.db.DB.Model(&model.ReferralCampaign{})
...
func (s *ReferralCampaignService) GetReferralCampaignByID(ctx context.Context, campaignID uint) (*model.ReferralCampaign, error) {
...
func (s *ReferralCampaignService) GetReferralCampaignByCode(ctx context.Context, code string) (*model.ReferralCampaign, error) {
	var campaign model.ReferralCampaign
...
func (s *ReferralCampaignService) UpdateReferralCampaign(ctx context.Context, campaignID uint, req *UpdateReferralCampaignRequest) (*model.ReferralCampaign, error) {
	var campaign model.ReferralCampaign
	if err := s.db.DB.First(&campaign, campaignID).Error; err != nil {
		return nil, fmt.Errorf("campaign not found: %w", err)
	}
...
func (s *ReferralCampaignService) GetActiveCampaigns(ctx context.Context) ([]*model.ReferralCampaign, error) {
	var campaigns []*model.ReferralCampaign
	
	now := time.Now()
	query := s.db.DB.Where("status = ? AND is_public = ?", model.CampaignStatusActive, true)
...
func (s *ReferralCampaignService) GetCampaignStats(ctx context.Context, campaignID uint) (map[string]interface{}, error) {
	...
	if err := s.db.DB.Model(&model.Referral{}).Where("campaign_id = ? AND status = ?", campaignID, model.ReferralStatusPending).Count(&pendingReferrals).Error; err != nil {
	...
	if err := s.db.DB.Model(&model.Referral{}).Where("campaign_id = ? AND status = ?", campaignID, model.ReferralStatusConfirmed).Count(&activeReferrals).Error; err != nil {
	...
	if err := s.db.DB.Model(&model.ReferralReward{}).Where("campaign_id = ? AND status IN (?)", campaignID, []string{model.RewardStatusPending, model.RewardStatusEarned}).Select("COALESCE(SUM(reward_amount), 0)").Scan(&pendingRewards).Error; err != nil {
