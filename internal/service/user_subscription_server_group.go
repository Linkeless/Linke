...
func (s *UserSubscriptionService) GetAvailableServerGroups(ctx context.Context, userID uint, subscriptionID uint) ([]*model.ServerGroup, error) {
	...
	var availableGroups []*model.ServerGroup
...
func (s *UserSubscriptionService) GetSubscriptionServerGroups(ctx context.Context, userID uint, subscriptionID uint) ([]*model.ServerGroup, error) {
	...
	if len(assignedGroupIDs) == 0 {
		return []*model.ServerGroup{}, nil
	}

	// Get server group details
	var serverGroups []*model.ServerGroup
...
func (s *UserSubscriptionService) UpdateSubscriptionServerGroups(ctx context.Context, userID uint, req *UpdateSubscriptionServerGroupsRequest) (*model.UserSubscription, error) {
	// Start transaction
	...
	if len(req.ServerGroupIDs) > 0 {
		...
		var existingGroups []model.ServerGroup
...
func (s *UserSubscriptionService) GetUserAccessibleServers(ctx context.Context, userID uint) ([]*model.ShadowsocksServer, error) {
	...
	if len(activeSubscriptions) == 0 {
		return []*model.ShadowsocksServer{}, nil
	}
	...
	var accessibleServers []*model.ShadowsocksServer
	if err := s.db.WithContext(ctx).
		Preload("ServerGroup").
...
func (s *UserSubscriptionService) GetUserServersBySubscription(ctx context.Context, userID uint, subscriptionID uint) ([]*model.ShadowsocksServer, error) {
	// Get the subscription to verify ownership
	subscription, err := s.GetUserSubscription(ctx, subscriptionID)
	if err != nil {
		return nil, fmt.Errorf("failed to get subscription: %w", err)
	...
	var servers []*model.ShadowsocksServer
	if err := s.db.WithContext(ctx).
		Preload("ServerGroup").
...
func (s *UserSubscriptionService) ValidateUserServerAccess(ctx context.Context, userID uint, serverID uint) (bool, *model.UserSubscription, error) {
	// Get the server to find its server group
	var server model.ShadowsocksServer
