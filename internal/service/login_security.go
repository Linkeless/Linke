...
func (l *LoginSecurityService) IsAccountLocked(ctx context.Context, email string) (bool, *model.AccountLockout, error) {
	var lockout model.AccountLockout
...
func (l *LoginSecurityService) GetFailureCount(ctx context.Context, email string) (int, error) {
	...
	var count int64
	err := l.db.WithContext(ctx).Model(&model.LoginAttempt{}).
...
func (l *LoginSecurityService) updateFailureTracking(ctx context.Context, email string, userID *uint) error {
	...
	if failureCount >= l.maxFailures && (lockout.LockedUntil == nil || lockout.LockedUntil.Before(now)) {
		...
		lockout.LockReason = model.LockReasonMultipleFailures
...
func (l *LoginSecurityService) resetFailureTracking(ctx context.Context, email string) error {
	return l.db.WithContext(ctx).Model(&model.AccountLockout{}).
		Where("email = ?", email).
...
func (l *LoginSecurityService) calculateLockoutDuration(lockout *model.AccountLockout) time.Duration {
	...
	l.db.Model(&model.LoginAttempt{}).
		Where("email = ? AND reason = ? AND created_at > ?", 
			lockout.Email, model.LoginFailureAccountLocked, time.Now().Add(-24*time.Hour)).
		Count(&lockCount)
...
func (l *LoginSecurityService) UnlockAccount(ctx context.Context, email string, reason string) error {
	result := l.db.WithContext(ctx).Model(&model.AccountLockout{}).
		Where("email = ?", email).
		Updates(map[string]interface{}{
			"locked_until": nil,
			"lock_reason":  "",
...
func (l *LoginSecurityService) GetLoginAttemptStats(ctx context.Context, since time.Time) (map[string]interface{}, error) {
	...
	if err := l.db.WithContext(ctx).Model(&model.LoginAttempt{}).
	...
	if err := l.db.WithContext(ctx).Model(&model.AccountLockout{}).
...
func (l *LoginSecurityService) CleanupOldAttempts(ctx context.Context, olderThan time.Duration) error {
	...
	result := l.db.WithContext(ctx).Where("created_at < ?", cutoff).Delete(&model.LoginAttempt{})
