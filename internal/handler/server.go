...
type ServerAPIHandler struct {
	shadowsocksService      *service.ShadowsocksServerService
...
func NewServerAPIHandler(shadowsocksService *service.ShadowsocksServerService, userService *service.UserService, userSubscriptionService *service.UserSubscriptionService, db *repository.Database, asynqClient *asynq.Client, cfg *config.Config) *ServerAPIHandler {
...
func (h *ServerAPIHandler) getUsersForServer(ctx context.Context, server *model.ShadowsocksServer) (*UniProxyUsersResponse, error) {
	...
	if err := h.db.DB.WithContext(ctx).
		...
		Where("status IN ?", []string{model.UserSubscriptionStatusActive, model.UserSubscriptionStatusTrial}).
	...
	var validSubscriptions []model.UserSubscription
	...
	if err := h.db.DB.WithContext(ctx).
		Select("id").
		Where("id IN ? AND status = ? AND deleted_at IS NULL", validUserIDs, model.UserStatusActive).
...
func (h *ServerAPIHandler) hasAccessToServerGroup(subscription *model.UserSubscription, groupID uint) bool {
