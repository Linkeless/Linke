...
func (s *UserSubscriptionService) GetUserSubscriptionStats(ctx context.Context, userID uint) (*SubscriptionStatsResponse, error) {
	...
	if err := s.db.WithContext(ctx).
		Model(&entities.SubscriptionOrder{}).
...
func (s *UserSubscriptionService) GetNotificationPreferences(ctx context.Context, userID uint) (*NotificationPreferences, error) {
	// Get user data using raw query to avoid cross-domain dependency
	var providerData *string
	if err := s.db.WithContext(ctx).
		Table("users").
		Select("provider_data").
		Where("id = ?", userID).
		Scan(&providerData).Error; err != nil {
...
// NOTE: ResetTrafficUsage method is now implemented in user_subscription.go
