...
// Fields are ordered for optimal memory alignment (8-byte aligned fields first)
type PaymentRecord struct {
	// 8-byte aligned fields (pointers and time.Time)
	...
	// 8-byte float64 and time.Time fields
	...
	// 8-byte for DeletedAt struct
	DeletedAt gorm.DeletedAt `json:"deleted_at,omitempty" gorm:"index"`

	// 8-byte uint fields (on 64-bit systems)
	ID     uint `json:"id" gorm:"primaryKey"`
	UserID uint `json:"user_id" gorm:"not null;index"`

	// 4-byte int fields
	NotifyCount int32 `json:"notify_count" gorm:"default:0"` // 通知次数，用于检测重复通知

	// String fields (grouped together to minimize padding)
	...
	// Large string fields at the end
...
// IncrementNotifyCount atomically increments the notification count
func (pr *PaymentRecord) IncrementNotifyCount() int32 {
	return atomic.AddInt32(&pr.NotifyCount, 1)
}

// GetNotifyCount atomically gets the current notification count
func (pr *PaymentRecord) GetNotifyCount() int32 {
	return atomic.LoadInt32(&pr.NotifyCount)
}
