...
type ReferralCampaign struct {
	...
	Referrals             []*Referral `json:"referrals,omitempty" gorm:"-"`
...
func (rc *ReferralCampaign) ToResponse() *ReferralCampaignResponse {
	...
	if rc.CreatedBy != nil {
		resp.CreatedBy = rc.CreatedBy.ToResponse()
	}
	if rc.Referrals != nil {
		for _, referral := range rc.Referrals {
			resp.Referrals = append(resp.Referrals, referral.ToResponse())
		}
	}
