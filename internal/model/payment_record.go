...
type PaymentRecordResponse struct {
	...
	SubscriptionOrder *SubscriptionOrderResponse `json:"subscription_order,omitempty"` // Subscription order info
...
func (pr *PaymentRecord) ToResponse() *PaymentRecordResponse {
	...
	if pr.User != nil {
		resp.User = pr.User.ToResponse()
	}
	if pr.SubscriptionOrder != nil {
		resp.SubscriptionOrder = pr.SubscriptionOrder.ToResponse()
	}
