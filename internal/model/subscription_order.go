...
type SubscriptionOrder struct {
	...
	SubscriptionPlan *SubscriptionPlan `json:"subscription_plan,omitempty" gorm:"-"`
	UserSubscription *UserSubscription `json:"user_subscription,omitempty" gorm:"-"`
...
func (so *SubscriptionOrder) ToResponse() *SubscriptionOrderResponse {
	...
	if so.User != nil {
		resp.User = so.User.ToResponse()
	}
	if so.SubscriptionPlan != nil {
		resp.SubscriptionPlan = so.SubscriptionPlan.ToResponse()
	}
	if so.UserSubscription != nil {
		resp.UserSubscription = so.UserSubscription.ToResponse()
	}
